function chinese(){
  ...
  function translate_chinese(){
    ...
    sub_gantry_crane_link.textContent = "电动葫芦"
    sub_gantry_crane_link.href = sub_gantry_crane_link.href + "#ch"

    var main_title = document.querySelector(".main_title")
    main_title.textContent = "钢丝绳电动葫芦"

    var def = document.querySelector(".def1")
    def.textContent = "CD、MD型钢丝绳电动葫芦是一种小型起重设备，可安装在单梁、桥式、门式和旋臂起重机上，稍加改装，也可用作绞盘。广泛应用于工厂、矿山港口、仓库、货物储存区和商店，对提高工作效率和改善工作条件至关重要。"

    var def = document.querySelector(".def2")
    def.textContent = "CD型电动葫芦只有一种正常速度，可以满足正常应用。"

    var def = document.querySelector(".def3")
    def.textContent = "MD电动葫芦提供两种速度；正常速度和低速，在低速时可以进行精确的装卸、沙箱的堆砌、机床的维修等。因此MD型电动葫芦比CD型更广泛地使用。"
  ...
  function translate_english(){
    ...
    var sub_gantry_crane_link = document.querySelector(".sub_gantry_crane_link")
    sub_gantry_crane_link.textContent = "Electric Hoist"
    sub_gantry_crane_link.href = sub_gantry_crane_link.href + "#eng"

    var main_title = document.querySelector(".main_title")
    main_title.textContent = "Wire Rope Electric Hoist"

    var def = document.querySelector(".def1")
    def.textContent = "Model CD, MD, Wire Rope Electric Hoist is small-sized lifting equipment.which can be mounted on a single beam, bridge, ganty and Jib cranes, With slight modification, it can also be used as a winch. It is widely used in factories, mines harbours, warehouses, cargo storage areas and shops, essential in raising working efficiency and improving working conditions."

    var def = document.querySelector(".def2")
    def.textContent = "Model CD Electric Hoist has only one normal speed, which can satisfy normal applications.Model"

    var def = document.querySelector(".def3")
    def.textContent = "MD Electric Hoist provides two speeds; normal speed and low speed, At a low speed, it can do precise loading and unloading, mounding of sand box, maintenance of machine tools, etc. Thus Model MD Electric Hoists are more widely used than Model CD."
