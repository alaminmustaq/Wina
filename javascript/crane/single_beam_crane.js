function chinese(){
  ...
  function translate_chinese(){
    ...
    main_title.textContent = "单梁起重机"

    var def = document.querySelector(".def1")
    def.textContent = "LD型单梁起重机的特点是结构更合理，钢材整体强度更高。与电动葫芦配套使用，是一种起重1-5吨的轻型起重机。跨度为7.5m-31.5m。工作等级为A3-A5。工作温度为-25℃至40℃。"

    var def = document.querySelector(".def2")
    def.textContent = "广泛用于厂房、仓库、物料库中起吊货物。禁止在易燃、易爆或有腐蚀性的环境中使用设备。主要的操作方式是地面控制（地面控制、遥控和机舱控制）。如果跨度和起升高度超出参数列出的范围，我们可以根据用户要求进行非标设计。"

    var spec = document.querySelector(".spec")
    spec.textContent = "主要特点："

    var spec_1 = document.querySelector(".spec_1")
    spec_1.textContent = "结构紧凑、刚性好、操作灵敏"

    var spec_2 = document.querySelector(".spec_2")
    spec_2.textContent = "节能，降低维护成本"

    var spec_3 = document.querySelector(".spec_3")
    spec_3.textContent = "软启动电机，降低冲击力"

    var spec_4 = document.querySelector(".spec_4")
    spec_4.textContent = "低建筑高度"

    var spec_4 = document.querySelector(".spec_5")
    spec_4.textContent = "硬壳式手推车和漂亮的外观"
  ...
  function translate_english(){
    ...
    var main_title = document.querySelector(".main_title")
    main_title.textContent = "Single Beam Crane"

    var def = document.querySelector(".def1")
    def.textContent = "LD model single beam crane is characterized by a more reasonable structure and higher strength steel as a whole. Used together with an electric hoist as a complete set, it is a light-duty crane with a capacity of 1 to 5 tons. The span is 7.5m-31.5m. The working grade is A3-A5. The working temperature is -25℃to 40℃."

    var def = document.querySelector(".def2")
    def.textContent = "It is widely used in plants, warehouses, and material stocks to lift goods. It is prohibited to use the equipment in the combustible, explosive or corrosive environment. The main operating method is ground control(ground control, remote control and cabin control). If the span and lifting height is out of the range that parameter lists, we can do non-standard design according to user requirement."

    var spec = document.querySelector(".spec")
    spec.textContent = "Main features:"

    var spec_1 = document.querySelector(".spec_1")
    spec_1.textContent = "Compact structure, good rigidity, and sensitive operation"

    var spec_2 = document.querySelector(".spec_2")
    spec_2.textContent = "Energy-saving and lower maintenance cost"

    var spec_3 = document.querySelector(".spec_3")
    spec_3.textContent = "Soft-start motor, lower the impact force"

    var spec_4 = document.querySelector(".spec_4")
    spec_4.textContent = "Low building height"

    var spec_4 = document.querySelector(".spec_5")
    spec_4.textContent = "Monocoque trolley and nice appearance"
